...
def get_int_matrix_from_string(grid):
    grid = grid.split("\n")                     # split rows
    grid = [row.split() for row in grid]        # split each row by space
    grid = [map(int, row) for row in grid]      # turn each str to a number in each row
    return grid
